...
const BACKEND_API = 'http://localhost:5000/api/crypto';
...
const TopMoverCard = ({ coin, currLabel }) => (
	<div className="flex flex-col items-center bg-white rounded-xl border border-gray-10 p-4 min-w-[120px] hover:shadow-elevation-1 transition-shadow duration-200 cursor-pointer">
		<img src={coin.image} 
		alt={coin.symbol} className="w-10 h-10 rounded-full mb-2" />
...
const NewCoinCard = ({ coin }) => (
	<div className="flex flex-col items-center bg-white rounded-xl border border-gray-10 p-4 min-w-[140px] hover:shadow-elevation-1 transition-shadow duration-200 cursor-pointer">
		<img src={coin.image} 
		alt={coin.name} className="w-10 h-10 rounded-full mb-2" />
		<p className="text-label-1 text-gray-100 uppercase">{coin.symbol}</p>
...
const TableLineChart = ({ changePct, coinId, symbol }) => {
	...
	return (
		<svg height="32" width="64"><g><path d={linePath} fill="transparent" stroke={color} strokeLinecap="round" strokeLinejoin="round" strokeWidth="2"></path></g></svg>
	);
};
...
const ExplorePage = () => {
	...
	useEffect(() => {
		...
		const loadData = async () => {
			...
			try {
				...
				if (assetFilter === 'gainers') endpoint += '/gainers';
				else if (assetFilter === 'new') endpoint += '/new';
				...
				if (!cancelled && result.status === 'success') {
					const rate = getRate();
					// Map backend fields to the property names used in this component
					const processed = result.data.coins.map(c => ({
						id: c._id,
						name: c.name,
						symbol: c.symbol,
						image: c.image,
						...
						// Since seeded data lacks these, we'll mock them relative to price
						market_cap: c.price * 1250000 * rate,
						total_volume: c.price * 85000 * rate,
					}));
					setCoins(processed);
					setLoading(false);
				}
	...
	const applyAssetFilter = (list) => {
		switch (assetFilter) {
			case 'tradeable':
				// Basic local filter since backend doesn't have "tradeable" yet
				return list.slice(0, 50);
			case 'losers':
				// Local filter for losers as backend only provides gainers
				return [...list]
